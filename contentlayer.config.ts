...
async function createTagCountForBriefs(allBriefs) {
  const tagCount: Record<string, number> = {}
  allBriefs.forEach((file) => {
    if (file.tags && (!isProduction || file.draft !== true)) {
      file.tags.forEach((tag) => {
        const formattedTag = slug(tag)
        if (formattedTag in tagCount) {
  ...
  const formatted = await prettier.format(JSON.stringify(tagCount, null, 2), { parser: 'json' })
  writeFileSync('./app/tag-data-briefs.json', formatted)
}
...
export const Blog = defineDocumentType(() => ({
  ...
  fields: {
    ...
    logo: { type: 'string' },
    pricing: { type: 'string' },
    pricingModel: { type: 'string' }, // Free, Freemium, Pay-as-you-go, Subscription
    targetUser: { type: 'string' },
    category: { type: 'string' }, // LLM, Image Gen, Coding, Productivity, Video, Audio
    rating: { type: 'number' },
    underlyingModel: { type: 'string' }, // GPT-4o, Claude 3.5, Llama 3, etc.
